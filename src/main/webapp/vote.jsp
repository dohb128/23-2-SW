...
.table-container {
    margin-left: 10px; /* 테이블 양 옆에 여백 설정*/
    margin-right: 10px; 
}
	...
	<thead>
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<form method="post" action="voteProccess.jsp">
						<input type="hidden" name="gameId"
							value="<%=resultSet.getInt("id")%>"> <input
							type="radio" name="selectedTeam"
							value="<%=resultSet.getString("team1")%>">
						<%=resultSet.getString("team1")%>
						<input type="radio" name="selectedTeam"
							value="<%=resultSet.getString("team2")%>">
						<%=resultSet.getString("team2")%>
						<input type="submit" value="투표" class="vote-button">
					</form>
				</td>
			</tr>
			<%
